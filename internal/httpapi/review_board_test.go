...
func TestResolveReviewBoardColumn(t *testing.T) {
	tests := []struct {
		...
		{name: "blocking findings", job: workflowjob.Job{Status: workflowjob.StatusWaitingApproval, ExecutionVersion: 1}, review: &reviewer.Run{Verdict: reviewer.VerdictRequestRevision, BlockingIssues: 1}, want: "ISSUES_FOUND"},
		...
		{name: "ready for approval", job: workflowjob.Job{Status: workflowjob.StatusWaitingApproval, ExecutionVersion: 1}, review: &reviewer.Run{Verdict: reviewer.VerdictApprove}, want: "READY_FOR_APPROVAL"},
