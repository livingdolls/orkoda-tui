...
func registerExecutionRoutes(api *gin.RouterGroup, registry ExecutionRegistry) {
	api.GET("/jobs/:jobID/executions", func(c *gin.Context) {
		if !requireExecutionRegistry(c, registry) { return }
		items, err := registry.ListWorkflow(c.Request.Context(), c.Param("jobID"))
		if err != nil { writeExecutionError(c, err); return }
...
func requireExecutionRegistry(c *gin.Context, registry ExecutionRegistry) bool {
	if registry == nil { writeError(c, http.StatusServiceUnavailable, "execution registry is unavailable"); return false }
	return true
}

func writeExecutionError(c *gin.Context, err error) {
	if errors.Is(err, execution.ErrNotFound) { writeError(c, http.StatusNotFound, err.Error()); return }
	if errors.Is(err, execution.ErrInvalid) { writeError(c, http.StatusBadRequest, err.Error()); return }
