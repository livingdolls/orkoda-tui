...
func (r *Repository) StartTool(ctx context.Context, executionID, tool string, input any, maxCalls int) (ToolRun, error) {
	...
	if err != nil {
		return ToolRun{}, fmt.Errorf("marshal tool input summary: %w", err)
	}
	now := r.now().UTC()
	...
	defer tx.Rollback()
	var count int
	if err := tx.QueryRowContext(ctx, `SELECT tool_calls FROM executions WHERE id=?`, executionID).Scan(&count); errors.Is(err, sql.ErrNoRows) {
		return ToolRun{}, ErrNotFound
	} else if err != nil {
		return ToolRun{}, err
	}
	if maxCalls > 0 && count >= maxCalls {
		return ToolRun{}, ErrToolCallLimit
	}
	sequence := count + 1
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO tool_runs (id,execution_id,sequence,tool,status,input_summary_json,output_summary_json,started_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)`, item.ID, item.ExecutionID, item.Sequence, item.Tool, item.Status, string(inputJSON), `{}`, now.UnixMilli(), now.UnixMilli(), now.UnixMilli()); err != nil {
		return ToolRun{}, err
	}
	if _, err := tx.ExecContext(ctx, `UPDATE executions SET tool_calls=?, updated_at=? WHERE id=?`, sequence, now.UnixMilli(), executionID); err != nil {
		return ToolRun{}, err
	}
...
func (r *Repository) SaveCheckpoint(ctx context.Context, executionID, baseSHA, headSHA, patch string, changedFiles []string) (Checkpoint, error) {
	...
	var sequence int
	if err := r.db.QueryRowContext(ctx, `SELECT COALESCE(MAX(sequence),0)+1 FROM patch_checkpoints WHERE execution_id=?`, executionID).Scan(&sequence); err != nil {
		return Checkpoint{}, err
	}
	now := r.now().UTC()
...
func (r *Repository) latestCheckpoint(ctx context.Context, executionID string) (Checkpoint, error) {
	items, err := r.ListCheckpoints(ctx, executionID)
...
func scanExecution(row interface{ Scan(...any) error }) (Execution, error) {
	...
	if completed.Valid {
		v := time.UnixMilli(completed.Int64).UTC()
		item.CompletedAt = &v
	}
	return item, nil
}
